...
\node[rec, anchor=north west] at (0,4.4)
 {state estimate $\shat$\\[2pt]
  ...
  {\color{ForestGreen!85!black}belief.iron.attr.qty: 5}\\
...
\node[rec, anchor=north west] at (4.85,3.9)
 {$\shat|_{\Rel(\bar\omega)}$\\[2pt]
  ...
  {\color{ForestGreen!85!black}iron.attr.qty: 5}};
...
\node[key, anchor=north west] at (8.95,3.9)
 {$\kappa=(\bar\omega,\ \sigma_{\bar\omega}(\shat))$\\[2pt]
  ...
  {\color{ForestGreen!85!black}iron.qty: $\ge$ required}};

\node[tlab, anchor=north west, align=left] at (11.9,3.9)
 {{\color{NavyBlue!85!black}$\Rel_{\mathrm{pre}}$: can it be executed}\\[2pt]
  {\color{ForestGreen!85!black}$\Rel_{\mathrm{eff}}$: what it yields}\\[2pt]
...
\node[tlab, anchor=north west, align=left, text width=6.9cm] at (7.6,-0.85)
 {outcome variance that co-varies with a feature outside $\Rel(\bar\omega)$ reveals an omitted precondition; the key is split between episodes};
\end{tikzpicture}
