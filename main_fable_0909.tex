...
\label{eq:belief}
\end{equation}
with fields: $\attr$, the element's attributes, comprising its type $\type(b)$, a unique identifier when the environment provides one, and mutable properties such as quantity; $\loc$, its location; $\tobs=(k,t,i)$, the episode, time step, and option index of last observation; $\prov\in\{\mathrm{observed},\mathrm{given},\mathrm{imagined}\}$, whether the belief derives from an observation, from the task description, or from the planner's prediction; $\vol\in\{\mathrm{permanent},\mathrm{semi\text{-}permanent},\mathrm{volatile}\}$, the expected persistence of the element (terrain; a container or structure; a mobile creature), assigned from the per-type summary $S(\type)$ maintained by the across-episode memory; and $\stat\in\{\mathrm{extant},\mathrm{consumed},\mathrm{unknown}\}$, where \emph{consumed} means the agent has depleted or removed the element and \emph{unknown} that the belief has been contradicted by a subsequent observation. 
...
$\stat$ & \emph{extant}, or \emph{unknown} for a \emph{given} belief without a location & \emph{extant}\,$\to$\,\emph{unknown} by the step update when $\loc$ is within sensing range and the element is absent, or by executor feedback; \emph{unknown}\,$\to$\,\emph{extant} on re-observation; \emph{extant}\,$\to$\,\emph{consumed} by consolidation when the option depleted the element; \emph{consumed} is terminal \\
\end{lifecycle}

\paragraph{Store.}
The Type 1 store at time $t$ is the belief set, $\calM_t=\calB_t$, keyed by $(\type,\loc)$. It holds no observation history: the step update below is recursive, so each observation is folded in once and then discarded. 
...
\label{eq:init}
\end{equation}
where the first term enters every element asserted by the task description as a belief with its stated location, if any, and $\mathrm{Seed}_k$ is a set of beliefs retained from the end of episode $k-1$ ($\mathrm{Seed}_1=\emptyset$). The retained beliefs are those with $\prov=\mathrm{observed}$, $\vol=\mathrm{permanent}$, and $\stat=\mathrm{extant}$, filtered by the across-episode memory as described in Section~\ref{sec:type1-type2}. Elements in the initial perceptual field are not beliefs; they become beliefs when they leave it.
...
\item \emph{Departure.} An element present in $\sP_{t-1}$ and absent from $\sP_t$ is written as a belief with the attributes and location last observed, $\tobs=(k,t,i)$, $\prov=\mathrm{observed}$, $\vol$ from $S(\type)$, and $\stat=\mathrm{extant}$.
...
\end{enumerate}
Updates are keyed by $(\type,\loc)$ rather than appended, so re-observing an element refreshes its belief rather than duplicating it. 
...
At episode termination, the beliefs eligible for seeding (observed, permanent, extant) are retained and all others are discarded. 
...
\label{eq:generator}
\end{equation}
The feasibility module judges an option that refers to a remembered element as feasible only if the node's belief component contains a belief for it with $\stat=\mathrm{extant}$ and a location; a belief with $\prov=\mathrm{imagined}$ or a heavily discounted $\tobs$ is reported to the module as such, and options resting on it are repaired into options that first verify the element.
...
\label{eq:reward}
\end{equation}
Besides feasibility, goal relevance, and coherence, $\calQ$ contains questions that credit an option for resolving uncertainty relevant to the goal (converting a belief with $\stat=\mathrm{unknown}$ into one with $\stat=\mathrm{extant}$ and a location) and debit an option that relies on an imagined belief.
...
\begin{itemize}[leftmargin=1.5em,itemsep=1pt]
\item a \emph{relevance set} $\Rel(\bar\omega)=\Rel_{\mathrm{pre}}(\bar\omega)\cup\Rel_{\mathrm{eff}}(\bar\omega)\subseteq\calF$, where $\Rel_{\mathrm{pre}}$ contains the features that condition whether the option can be executed (required possessions, required tool tier, the status and location of the element it refers to) and $\Rel_{\mathrm{eff}}$ the features that modulate its effect (the quantity available at the element, the agent's carrying capacity); and
\item a \emph{value abstraction} $\alpha_{\bar\omega}$ that maps the raw value of each feature in $\Rel(\bar\omega)$ to an equivalence class: quantities to threshold buckets relative to the option's requirement, tool-like attributes to tiers, locations to $\{\mathrm{known},\mathrm{unknown}\}$, belief status to $\{\mathrm{extant},\mathrm{consumed},\mathrm{unknown}\}$.
\end{itemize}
