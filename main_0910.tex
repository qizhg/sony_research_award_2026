...
\label{eq:belief}
\end{equation}
with fields: $\attr$, the element's attributes, comprising its type $\type(b)$, a unique identifier when the environment provides one, and mutable properties such as quantity; $\loc$, its location; $\tobs=(k,t,i)$, the episode, time step, and option index of last observation; $\prov\in\{\mathrm{observed},\mathrm{given},\mathrm{imagined}\}$, whether the belief derives from an observation, from the task description, or from the planner's prediction; $\vol\in\{\mathrm{permanent},\mathrm{semi\text{-}permanent},\mathrm{volatile}\}$, the expected persistence of the element (terrain; a container or structure; a mobile creature), assigned from the per-type summary $S(\type)$ maintained by the across-episode memory; and $\stat\in\{\mathrm{extant},\mathrm{consumed},\mathrm{unknown}\}$, 
where \emph{extant} means the element is currently present and \emph{consumed} means the agent has depleted or removed the element and \emph{unknown} that the belief has been contradicted by a subsequent observation. 
...
\label{eq:generator}
\end{equation}
conditioned on the node and therefore on its belief component as well as on its perceived and agent components. An option may consequently refer to a remembered element and not only to a perceived one: a belief with $\stat=\mathrm{extant}$ and a location supports an option that goes directly to the element, while a belief with $\prov=\mathrm{imagined}$ or a heavily discounted $\tobs$ is presented to the generator as such, so that what it proposes on the strength of that belief is an option that verifies the element first. Candidates are not filtered at proposal time; whether an option is worth executing from the node's state is settled by the reward predictor below and by the search.
